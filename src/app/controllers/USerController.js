...
class UserController {
  async store(req, res) {
    ...
    const {
      id, name, email, prestador_servico,
  ...
  async update(req, res) {
    ...
    const esquema = Yup.object().shape({
      ...
      password: Yup.string()
        .min(6)
        .when('passwordAntigo', (passwordAntigo, field) => (passwordAntigo ? field.required() : field)), // validação condicional
      confirmarPassword: Yup.string().when('password', (password, field) => (password ? field.required().oneOf([Yup.ref('password')]) : field)), // validar o novo password
    });
